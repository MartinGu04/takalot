...
-- "הערה נוספת" (additional note): one new optional, free-text, ≤600-character
-- user note on incident creation, full update, technician update, and
-- closure (both readiness outcomes).
--
-- Storage: a NEW `user_note` column on `incident_events` and on
-- `incident_updates`, never a reuse of any existing column:
--   - incident_events.note already carries GENERATED narrative text on
--     create_incident's 'created' row (actions taken / comms / WISDOM
--     summary) and on close_incident's 'closed' / partial-readiness
--     'status_change' row (root cause / resolution / follow-up summary) --
--     writing a raw user note into that column would silently overwrite or
--     ambiguously concatenate with that generated text.
--   - incident_events.note also already carries a raw but semantically
--     DIFFERENT value on update_incident's 'status_change'/'severity_change'
--     rows: `changeReason`, the reason for that specific structured change,
--     not a general note on the update.
--   - incident_updates has no spare text column: actions_taken, findings,
--     next_steps, and current_status_text are all already required/optional
--     raw user fields with their own distinct meaning.
-- A dedicated `user_note` column on each table keeps the new note
-- unambiguous and guarantees it can never overwrite or mix with any
-- existing generated or raw text, on any of the four RPCs.
--
-- Both tables are append-only (trg_incident_events_immutable /
-- trg_incident_updates_immutable, 0001) -- user_note is therefore
-- permanently NULL on every row written before this migration, exactly like
-- operation_id (0025) before it, and a later action's note always lives on
-- ITS OWN new row; it never modifies an earlier one.
--
-- assign_incident is NOT touched by this migration: it already accepts and
-- stores a raw, optional client note directly in incident_events.note for
-- its 'assignment_change' event (that row never carries generated content),
-- and reassignment is not one of the four flows in scope here.

alter table incident_events
  add column user_note text
    check (user_note is null or length(user_note) <= 600);
...
-- =====================================================================
-- 1. create_incident -- unchanged since 0037 except the additions below
--    (v_user_note declared/validated, stored on the 'created' event row
--    alongside its existing generated note, never replacing it).
-- =====================================================================
...
-- =====================================================================
-- 2. update_incident -- unchanged since 0032 except the additions below
--    (v_user_note declared/validated, stored on the incident_updates row --
--    the primary 'update' event row never carried a note and still doesn't).
-- =====================================================================
...
-- =====================================================================
-- 3. technician_update_incident -- unchanged since 0029 except the
--    additions below. No permission/ownership/terminal-state change: this
--    RPC still only ever touches incident_updates content-only fields plus
--    the incident's version/updated_by/last_update_at -- adding user_note
--    does not grant any protected-field capability.
-- =====================================================================
...
-- =====================================================================
-- 4. close_incident -- unchanged since 0037 except the additions below
--    (v_user_note declared/validated, stored on the primary closure/
--    partial-readiness event row alongside its existing generated note,
--    in both branches, never replacing it).
-- =====================================================================
