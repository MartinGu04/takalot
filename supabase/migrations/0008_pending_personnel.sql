...
-- reads the VERIFIED email server-side from auth.users, and atomically
-- claims the matching pending entry: links the auth identity, creates the
-- real public.profiles row with the preassigned role, marks the entry
-- claimed, and returns the profile. No invitation link, no manual UUID
-- handling, no Supabase-dashboard step. Anything that does not match
-- exactly (no entry, cancelled, expired, already claimed, different
-- account) fails closed and the user stays unauthorized.
--
...
-- ===== 1. Schema =====
create type pending_personnel_status as enum ('pending', 'claimed', 'cancelled');
...
create table pending_personnel (
  ...
  status pending_personnel_status not null default 'pending',
...
-- email. Claimed/cancelled rows are history and do not block re-inviting.
create unique index idx_pending_personnel_email_claimable
  on pending_personnel (email) where status = 'pending';
create index idx_pending_personnel_status on pending_personnel (status, created_at desc);
...
alter table pending_personnel enable row level security;
create policy pending_personnel_select on pending_personnel for select
  using (my_role() in ('shift_supervisor', 'professional_manager', 'system_admin'));
...
create or replace function role_ceiling_allows(p_creator app_role, p_target app_role) returns boolean
...
create or replace function create_pending_personnel(p_input jsonb) returns pending_personnel
language plpgsql security definer set search_path = public as $$
declare
  v_role app_role := my_role();
  v_target app_role := (p_input->>'role')::app_role;
  v_email text := lower(trim(coalesce(p_input->>'email', '')));
  v_name text := trim(coalesce(p_input->>'fullName', ''));
  v_row pending_personnel;
begin
  if v_role is null or not role_ceiling_allows(v_role, v_target) then
  ...
  -- An email already linked to an existing (provisioned) account must not
  -- get a second, conflicting pending identity.
  if exists (
  ...
  begin
    insert into pending_personnel (full_name, email, role, created_by)
    values (v_name, v_email, v_target, auth.uid())
    returning * into v_row;
  exception when unique_violation then
    raise exception 'validation: כבר קיים רישום ממתין לכתובת דוא״ל זו';
  end;

  perform write_audit('personnel_pending_created', 'pending_personnel', v_row.id::text, null,
    null, jsonb_build_object('email', v_row.email, 'role', v_row.role, 'fullName', v_row.full_name));
  return v_row;
end;
$$;

create or replace function update_pending_personnel(p_id uuid, p_input jsonb) returns pending_personnel
language plpgsql security definer set search_path = public as $$
declare
  v_role app_role := my_role();
  v_target app_role := (p_input->>'role')::app_role;
  v_email text := lower(trim(coalesce(p_input->>'email', '')));
  v_name text := trim(coalesce(p_input->>'fullName', ''));
  v_old pending_personnel;
  v_row pending_personnel;
begin
  select * into v_old from pending_personnel where id = p_id for update;
  if not found then
  ...
  if v_role is null
     or not role_ceiling_allows(v_role, v_old.role)
     or not role_ceiling_allows(v_role, v_target) then
  ...
  begin
    update pending_personnel
      set full_name = v_name, email = v_email, role = v_target, updated_at = now()
      where id = p_id
      returning * into v_row;
  exception when unique_violation then
    raise exception 'validation: כבר קיים רישום ממתין לכתובת דוא״ל זו';
  ...
  perform write_audit('personnel_pending_updated', 'pending_personnel', p_id::text, null,
...
create or replace function cancel_pending_personnel(p_id uuid) returns void
language plpgsql security definer set search_path = public as $$
declare
  v_role app_role := my_role();
  v_old pending_personnel;
begin
  ...
  if v_role is null or not role_ceiling_allows(v_role, v_old.role) then
  ...
  update pending_personnel
    ...
    where id = p_id;
  perform write_audit('personnel_pending_cancelled', 'pending_personnel', p_id::text, null,
...
-- auth.uid(), the email from auth.users for that identity. Race-safe: the
-- pending->claimed transition is a single conditional UPDATE (concurrent
-- claimers serialize on the row lock; exactly one sees status='pending'),
-- and the whole function is one transaction -- a failure after the claim
-- rolls the claim back too. Privilege-safe: the created profile's role
-- comes only from the pending row, which itself could only be written
-- under the role ceilings above.
create or replace function claim_pending_personnel() returns profiles
language plpgsql security definer set search_path = public as $$
declare
  v_uid uuid := auth.uid();
  v_email text;
  v_entry pending_personnel;
  v_profile profiles;
begin
  ...
  -- return the existing profile; never claim twice for one identity.
  select * into v_profile from profiles where id = v_uid;
  if found then
    return v_profile;
  end if;

  -- The VERIFIED email, read server-side. Client input plays no part.
  ...
  if not found then
    ...
    -- of this user just claimed and created the profile, return it.
    select * into v_profile from profiles where id = v_uid;
    if found then
      return v_profile;
    end if;
  ...
  perform write_audit('personnel_pending_claimed', 'pending_personnel', v_entry.id::text, null,
...
-- ===== 5. Grants =====
-- Every new function: nothing for PUBLIC/anon. Management RPCs and the
-- claim RPC are callable by authenticated (their bodies enforce the real
-- authorization); the ceiling helper is internal only.
revoke execute on function public.role_ceiling_allows(app_role, app_role) from public, anon, authenticated;
...
revoke execute on function public.admin_create_placeholder_profile(text, app_role) from public, anon, authenticated;
comment on table placeholder_profiles is
  'DEPRECATED (0008): superseded by pending_personnel. Historical rows retained; no longer written.';
comment on function admin_create_placeholder_profile(text, app_role) is
