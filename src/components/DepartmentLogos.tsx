    ...
    >
      {DEPARTMENT_LOGOS.map((logo) => (
        <span
          ...
          className="flex size-8 shrink-0 items-center justify-center border border-hairline-strong bg-surface lg:size-[37px]"
        ...
        >
          {/* The badge shrank (~11-12%) while the artwork itself should read
              larger inside it, so the image is deliberately sized a bit
              past its own circle (112%, same on both axes -- this scales
              the box the source image sits in, never the image file, and
              scaling both axes equally can never skew its aspect ratio)
              rather than filling it exactly. The parent's overflow:hidden +
              circular mask trims only the excess margin the source frame
              already carries around the mark, the same way it already
              trimmed the square corners, leaving a small even gap between
              the mark itself and the border on every side. object-contain
              still governs how the (now larger) image box itself renders
              the source file, so it's still never stretched or cropped. */}
          <img
            src={logo.src}
            alt=""
