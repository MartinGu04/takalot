...
export default function HandoverDetailPage() {
  ...
  return (
    <div className="mx-auto max-w-2xl pb-12">
      ...
      <h2 className="mt-6 mb-2 text-lg font-bold">תקלות שנכללו ({items.length})</h2>
        ...
        {items.map((item) => (
          ...
          >
            <div className="flex flex-wrap items-center gap-2">
              <span className="font-bold text-blue-700 dark:text-blue-400">{item.snapshotNumber}</span>
            ...
            {item.note && <p className="mt-1 text-sm font-medium text-blue-800 dark:text-blue-300">הערת מסירה: {item.note}</p>}
          </Link>
        ))}
      </div>
      ...
      {handover.status === 'accepted' && (
        <section className="mt-6">
          <h2 className="mb-2 text-lg font-bold">תוספות</h2>
          {addenda.map((a) => (
            <div key={a.id} className="mb-2 rounded-lg bg-neutral-50 p-3 text-sm dark:bg-neutral-800/60">
              <p className="text-xs text-neutral-500">{name(a.authorId)} · {formatDateTime(a.createdAt)}</p>
